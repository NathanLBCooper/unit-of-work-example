...
public class TestDatabaseContext : IDisposable
{
    ...
    private static void CreateTestDatabase(string databaseName)
    {
        ...
        _ = defaultDbConnection.Execute($"create database {databaseName}");
    }
    ...
    public void Dispose()
    {
        ...
        _ = defaultDbConnection.Execute($"SELECT pg_terminate_backend(pg_stat_activity.pid) FROM pg_stat_activity WHERE pg_stat_activity.datname = '{_databaseName}' AND pid <> pg_backend_pid()");
        _ = defaultDbConnection.Execute($"DROP DATABASE IF EXISTS {_databaseName}");
    }
